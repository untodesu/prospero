...
// File: string.cc; Created: Mon Nov 24 2025 22:29:36
...
bool core::is_whitespace(std::string_view string)
{
    auto result = false;
...
bool core::has_whitespace(std::string_view string)
{
    return string.find_first_of(WHITESPACE_CHARS) != std::string_view::npos;
}

std::string_view core::skip_whitespace(std::string_view string)
{
...
std::string_view core::trim_whitespace(std::string_view string)
{
...
bool core::tokenize(std::string_view string, std::vector<std::string_view>& parts)
{
    parts.clear();

    while(string.size()) {
        string = core::skip_whitespace(string);
