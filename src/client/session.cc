...
void Session::handle_packet(const ENetPacket* packet)
{
    ...
    thread_local ImageMessage image_message;
    ...
    if(m_aes_context) {
        switch(packet_type) {
            ...
            case ImageMessage::ID:
                ImageMessage::deserialize(m_aes_context, buffer, image_message);
                handle_image_message(image_message);
                break;
...
void Session::handle_text_message(const TextMessage& packet)
{
    ...
    auto timestamp = QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(packet.timestamp), QTimeZone::UTC);
    auto username = QString::fromStdString(packet.username.substr(0U, MAX_USERNAME_LENGTH));
    auto message = QString::fromStdString(packet.message);

    emit text_message_received(timestamp, username, message);
}

void Session::handle_image_message(const ImageMessage& packet)
{
    assert(m_aes_context);

    auto timestamp = QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(packet.timestamp), QTimeZone::UTC);
    auto username = QString::fromStdString(packet.username.substr(0U, MAX_USERNAME_LENGTH));
    auto source = QString::fromStdString(packet.source);

    emit image_message_received(timestamp, username, source);
}
